class EpiCtsIdentifiersController < IdentifiersController
  ...
  def update
    ...
    if params[:commit] == "Save With Broken Leiden+" #Save With Broken Leiden+ button is clicked
      @identifier.save_broken_leiden_plus_to_xml(params[:epi_cts_identifier_leiden_plus], params[:comment])
      @bad_leiden = true
      ...
      begin
        commit_sha = @identifier.set_leiden_plus(params[:epi_cts_identifier_leiden_plus],
                                    params[:comment])
