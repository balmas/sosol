class EpiTransCtsIdentifiersController < IdentifiersController
  ...
  def update
    ...
    if params[:commit]== "Save With Broken Leiden+"
     #broken leiden
      @identifier.save_broken_leiden_trans_to_xml(params[:epi_trans_cts_identifier][:leiden_trans], params[:comment])
      @bad_leiden = true
      flash.now[:notice] = "File updated with broken Leiden+ - XML and Preview will be incorrect until fixed"
      expire_publication_cache
        @identifier[:leiden_trans] = params[:epi_trans_cts_identifier][:leiden_trans]
      ...
      begin#checking for parse error
        ...
        commit_sha = @identifier.set_leiden_translation_content(params[:epi_trans_cts_identifier][:leiden_trans], params[:comment])
