include HgvMetaIdentifierHelper  #TODO modify as necessary for Epigraphy sources
class EpiMetaCiteIdentifiersController < IdentifiersController
  layout 'site'
  ...
  def update
    ...
    #exit
    begin
      commit_sha = @identifier.set_epidoc(params[:epi_meta_cite_identifier], params[:comment])
  ...
  def get_date_preview
    ...
    [:X, :Y, :Z].each{|dateId|
     index = ('X'[0] - dateId.to_s[0]).abs.to_s
       if params[:epi_meta_cite_identifier][:textDate][index]
         @updates[dateId] = {
           :when      => params[:epi_meta_cite_identifier][:textDate][index][:attributes][:when],
           :notBefore => params[:epi_meta_cite_identifier][:textDate][index][:attributes][:notBefore],
           :notAfter  => params[:epi_meta_cite_identifier][:textDate][index][:attributes][:notAfter],
           :format   => params[:epi_meta_cite_identifier][:textDate][index][:value]
         }
       end
    }
  ...
  def get_geo_preview
    @identifier = EpiMetaCITEIdentifier.new
    @identifier.populate_epidoc_attributes_from_attributes_hash params[:epi_meta_cite_identifier]
    @update = EpiProvenance.format @identifier[:origPlace], @identifier[:provenance]
  ...
  protected
    ...
    def prune_params
      ...
      if params[:epi_meta_cite_identifier]

        # get rid of empty publication parts
        if params[:epi_meta_cite_identifier][:publicationExtra]
          params[:epi_meta_cite_identifier][:publicationExtra].delete_if{|index, extra|
            extra[:value].empty?
          }
        end

        if params[:epi_meta_cite_identifier][:textDate]
          
          # get rid of empty (invalid) date items
          params[:epi_meta_cite_identifier][:textDate].delete_if{|index, date|
            date[:c].empty? && date[:y].empty? && !date[:unknown]
          }

          # get rid of unnecessary date attribute @xml:id if there is only one date
          if params[:epi_meta_cite_identifier][:textDate].length == 1
            params[:epi_meta_cite_identifier][:textDate]['0'][:attributes][:id] = nil
          end
        end

        # get rid of empty certainties for mentioned dates (X, Y, Z)
        if params[:epi_meta_cite_identifier]['mentionedDate']
          params[:epi_meta_cite_identifier]['mentionedDate'].each_pair{|index, date|
    ...
    def complement_params
      ...
      if params[:epi_meta_cite_identifier]
        ...
        if params[:epi_meta_cite_identifier][:textDate]
          params[:epi_meta_cite_identifier][:textDate].each{|index, date| # for each textDate, i.e. X, Y, Z
            date[:id] = date[:attributes][:id]
            date.delete_if {|k,v| !v.instance_of?(String) || v.empty? }
            params[:epi_meta_cite_identifier][:textDate][index] = HgvDate.hgvToEpidoc date
          }
        end
        
        if params[:epi_meta_cite_identifier][:mentionedDate]
          params[:epi_meta_cite_identifier][:mentionedDate].each{|index, date|
    ...
    def find_identifier
      @identifier = EpiMetaCITEIdentifier.find(params[:id])
    end
